...
def resiBlock_2D_context(x, filter_nums):
    ...
    w = basic_DL_op.weight_variable('conv1_c', [3, 3, filter_nums, filter_nums], 0.01)
    ...
    w = basic_DL_op.weight_variable('conv2_c', [3, 3, filter_nums, filter_nums], 0.01)
...
def mask_2D_layer(x, entropy_resi_num, out_dim, mag, context):
    ...
    w_c = basic_DL_op.weight_variable('conv1_c', [3, 3, in_dim, int(5 * out_dim)], 0.01)
    ...
    b_c = basic_DL_op.bias_variable('bias1_c', [int(5 * out_dim)])
    ...
    for i in range(entropy_resi_num):
        with tf.variable_scope('resi_block' + str(i)):
            ...
            context = resiBlock_2D_context(context, int(5 * out_dim))
            x = x + mag + context
